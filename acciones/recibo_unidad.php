...
<body>
<div class="container">
  <h3 class="text-center mb-4">Recibo de Unidad - <?= $maquinaria['nombre'] ?></h3>
  ...
  <div class="mb-3">
    <strong>Modelo:</strong> <?= $maquinaria['modelo'] ?> <br>
    <strong>Ubicación:</strong> <?= $maquinaria['ubicacion'] ?> <br>
    <strong>Tipo:</strong> <?= $maquinaria['tipo_maquinaria'] ?> |
    <strong>Subtipo:</strong> <?= $maquinaria['subtipo'] ?>
  </div>
  ...
  <form method="POST" action="guardar_recibo.php?id=<?= $id_maquinaria ?>" id="recibo_unidad">
    ...
    <div class="seccion">
      ...
      <div class="progress mb-3">
        <div id="<?= $id_barra ?>" class="progress-bar bg-success barra-avance" style="width: 0%">0%</div>
      </div>
      <div class="row">
        ...
        <div class="col-md-6 mb-2">
          ...
          <div class="btn-group btn-group-md" role="group">
            ...
            ?>
            <button type="button" class="btn btn-opcion btn-md <?= $btn_class ?>" onclick="seleccionar('<?= $id_input ?>','<?= $opcion ?>', this)">
              <?= ucfirst($opcion) ?>
            </button>
            <?php endforeach; ?>
          </div>
    ...
    <div class="total-condicion my-4">
...
function calcularAvance() {
  ...
  for (const seccion in secciones) {
    ...
    const peso = pesos[seccion];
    total += (buenos / inputs.length) * peso;
